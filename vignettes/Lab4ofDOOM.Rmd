...
- `plot()`: this will plot the residuals vs. fitts and the standardized residuals vs fitts. The plots are generated with the package `ggplot2`. How these plots will look like are presented in the Plots chapter, little further down.
...
With the `linreg$new(formula, data)$plot()` method you will get the two plots shown here:
...
plotting <- linreg$new(Petal.Length~Sepal.Width+Sepal.Length, data=iris)$plot()

plotting[[1]]; plotting[[2]]
```
